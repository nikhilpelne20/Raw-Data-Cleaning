...
def data_cleaning(df):
    ...
    df_clean_time = df_clean_col_sub_str.withColumn(
        "time",
        (coalesce(regexp_extract("time", r"(\d+)\s*hrs?", 1).cast("int"), lit(0)) * 60) +
        coalesce(regexp_extract("time", r"(\d+)\s*mins?", 1).cast("int"), lit(0))
    )

    df_clean_stars = df_clean_time.withColumn("star_ratings",
                                              regexp_extract("stars", r"(\d+\.?\d*)\s*out\s*of\s*5\s*stars", 1)) \
        .withColumn("ratings", regexp_extract(col("stars"), r"(\d+)\s*ratings", 1))

    df_remove_empty_val = df_clean_stars.withColumn("star_ratings",
                                                    when(col("star_ratings") == "", "0").otherwise(col("star_ratings"))) \
        .withColumn("ratings", when(col("ratings") == "", "0").otherwise(col("ratings")))

    df_cleaned_data = df_remove_empty_val.select(
        "name", "author", "narrator", "time", "releasedate", "language",
        col("star_ratings").alias("stars"), "price", "ratings"
    )
